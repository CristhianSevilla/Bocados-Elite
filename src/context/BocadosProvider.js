...
const BocadosProvider = ({ children }) => {
  ...
  //Calcular Total
  useEffect(() => {
    const totalFinal = pedido.reduce(
  ...
  const handleAgregarPedido = ({ categoriaId, ...producto }) => {
    ...
    if (pedido.some((productoState) => productoState.id === producto.id)) {
      ...
      //Mostrar Notificación
      toast.success(
        `¡${producto.cantidad} Bocadito${
          producto.cantidad > 1 ? "s" : ""
        } Agregado${producto.cantidad > 1 ? "s" : ""} al Pedido!`
      );
    }
  ...
  const handleEditarCantidad = (id) => {
    const productoActuActualizar = pedido.filter(
